...
# My solution (NÃO FUNCIONOU EM TODOS OS TESTE!)
def dirReduc(arr)
  result = []
  if arr.count('NORTH') > arr.count('SOUTH')
    (arr.count('NORTH') - arr.count('SOUTH')).times do
      result << 'NORTH'
    end
  elsif arr.count('NORTH') < arr.count('SOUTH')
    (arr.count('SOUTH') - arr.count('NORTH')).times do
      result << 'SOUTH'
    end
  end
  if arr.count('EAST') > arr.count('WEST')
    (arr.count('EAST') - arr.count('WEST')).times do
      result << 'EAST'
    end
  elsif arr.count('EAST') < arr.count('WEST')
    (arr.count('WEST') - arr.count('EAST')).times do
      result << 'WEST'
    end
  end
  result
